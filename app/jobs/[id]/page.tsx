...
export default async function JobDetailPage({ params }: { params: Params }) {
  ...
  const { data: job } = await supabase
    .from("jobs")
    .select(
      "id, title, description, job_type, category, pay_min, pay_max, pay_period, employer_id, profiles ( id, full_name, messenger_username )",
    )
    .eq("id", id)
    .single();
      ...
      <div className="md:col-span-2 space-y-5">
        ...
        {employer && (
          <div className="flex items-center gap-4 flex-wrap">
            ...
            >
              posted by {employer.full_name ?? "an employer"}
            </Link>
